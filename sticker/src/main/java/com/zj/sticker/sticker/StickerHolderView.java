...
public class StickerHolderView extends RelativeLayout {
     ...
     */
    public void addStickerView(SelectHolderImp selectHolderImp, @NonNull TextStickerConfig config) {
        if (config.getText().trim().length() != 0) {
            addStickerView(selectHolderImp, config, true);
        }
    }
     ...
     * @param config sticker configuration.
     */
    public void addStickerView(SelectHolderImp selectHolderImp, ImageStickerConfig config) {
        addStickerView(selectHolderImp, config, true);
    }
     ...
     * @param config sticker configuration.
     */
    private void addStickerView(SelectHolderImp selectHolderImp, StickerConfig config, boolean isNew) {
        final StickerView sticker = new StickerView(this.getContext(), config, this);

        sticker.setSelectHolderImp(selectHolderImp);
    ...
    public void deleteSticker() {
        if(currentStickerView != null) {
            ...
            StickerConfig config = currentStickerView.getConfig();
            if (config != null) {
            ...
            currentStickerView.removeSelectHolder();
...
//                    new TextChangeDialog(getContext(), currentStickerView).show();
    ...
    @Nullable
    public StickerConfig getCurrentStickerConfig() {
    ...
    @Nullable
    public synchronized StickerView setCurrentEdit(@Nullable StickerView stickerView, final boolean isNew) {
        ...
        if (stickerView == null) {
            ...
            if (callback != null) {
                if (currentStickerView.getConfig().getType() == StickerConfig.StickType.TEXT) {
